...
class Text extends Koken {
	...
	function listing($params)
	{
		...
		$this->include_related_count('albums', null, array('listed' => 1));
		$this->include_related_count('categories');

		if ($options['order_by'] === 'title' && !isset($params['order_direction']))
		{
	...
	function to_array($options = array())
	{
		...
		$exclude = array('deleted', 'total_count', 'video_count', 'audio_count', 'featured_image_id', 'custom_featured_image', 'tags_old');
