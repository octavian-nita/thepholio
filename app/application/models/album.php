...
class Album extends Koken {
	...
	function update_counts($save = true, $special_case = false)
	{
		...
		if ($special_case)
		{
			if ($special_case['visibility'] === 'public')
			{
				$c->group_start()
					->where('visibility <', $this->listed == 0 ? 2 : 1)
					->or_where('id', $special_case['id'])
				->group_end();
			}
			else
			{
				$c->where('visibility <', $this->listed == 0 ? 2 : 1);
	...
	function listing($params)
	{
		...
		if (isset($params['order_by']) && !isset($params['order_direction']) && in_array($params['order_by'], array('created_on', 'modified_on', 'published_on', 'total_count', 'image_count', 'video_count')))
		{
	...
	function manage_content($content_id, $method = 'post', $match_album_visibility = false)
	{
		...
		if ($this->album_type == 0)
		{
			...
			if (count($added_ids) && !is_null($this->listed) && $match_album_visibility)
			{
				$change = new Content;
				$from = 0;
				$to = 1;
				...
				$change->where_in('id', $added_ids)->where('visibility', $from)->update(array('visibility' => $to));
			}
			...
			switch($method)
			{
				...
				case 'delete':
					...
					foreach($ids as $move_id)
					{
						...
						$a->where('listed', 1)->where('published_on', NULL)->update(array('published_on' => time()));
	...
	function to_array($options = array())
	{
		...
		$exclude = array('deleted', 'total_count', 'video_count', 'featured_order', 'tags_old');
		...
		if ($options['with_covers']) {
			...
			$covers->include_related_count('albums', NULL, array('listed' => 1));
