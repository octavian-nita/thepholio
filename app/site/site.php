<?php
	...
	if ($_SERVER['REQUEST_METHOD'] === 'GET' && (!$draft || $preview) && !isset($_GET['default_link']) && !isset($_COOKIE['share_to_tumblr']))
	{
		...
		$cache_path = $root_path . DIRECTORY_SEPARATOR . 'storage' . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . 'site' . str_replace('/', DIRECTORY_SEPARATOR, $cache_url) . ( $css || $js || preg_match('/\.rss$/', $cache_url) ? '' : ( $pjax ? '.phtml' : '.html' ) );
	...
	@include_once $root_path . '/storage/configuration/user_setup.php';
	...
	@include_once $root_path . '/storage/configuration/key.php';
	$video = Koken::api('/content/types:video/limit:1/visibility:any/token:' . $KOKEN_ENCRYPTION_KEY);
