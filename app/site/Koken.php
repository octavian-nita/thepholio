...
META;
	...
	public static function link($parameters)
	{
		...
		if ($options['share'])
		{
			...
			$attributes['title'] = 'Share &quot;' . self::out($title_getter . ' html_encode="true"') . '&quot; on ' . $label;
			...
			{
				$image = urlencode($token['presets']['large']['url']);
			}
			else if (isset($token['album']) && count($token['album']['covers']))
			{
				$image = urlencode($token['album']['covers'][0]['presets']['large']['url']);
			}
			else if (isset($token['featured_image']) && $token['featured_image'])
			{
				$image = urlencode($token['featured_image']['presets']['large']['url']);
			}
			...
			switch ($options['share']) {
				...
				case 'tumblr':
					$base_url = 'http://www.tumblr.com/share/photo';
					...
					$share_parameters['click_thru'] = $share_url;

					if ($image)
					{
						$share_parameters['source'] = $image;
					}
					break;
